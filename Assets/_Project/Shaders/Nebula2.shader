    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float r = BitangentNoise4D(float4(i.pos.xyz * _Frequency, _OffsetR + time))  * _Power + _ColorOffset;
                float g = BitangentNoise4D(float4(i.pos.xyz * _Frequency, _OffsetG + time))  * _Power + _ColorOffset;
                float b = BitangentNoise4D(float4(i.pos.xyz * _Frequency, _OffsetB + time))  * _Power + _ColorOffset;
                float a = BitangentNoise4D(float4(i.pos.xyz * _FrequencyA, _OffsetA + time)) * _Power + _ColorOffset;
